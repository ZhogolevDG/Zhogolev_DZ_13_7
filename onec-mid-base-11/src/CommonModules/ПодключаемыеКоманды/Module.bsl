...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыРазмещения = Неопределено) Экспорт
    ...
    //++ Жоголев Д Задача: Добаление реквизитов на форму 27.02.25	
	Если ЭтоФормаОбъекта Тогда         
		Если ИмяФормы = "Документ.ЗаказПокупателя.Форма.ФормаДокумента" ИЛИ ИмяФормы = "Документ.ПоступлениеТоваровУслуг.Форма.ФормаДокумента"                          
			ИЛИ ИмяФормы = "Документ.РеализацияТоваровУслуг.Форма.ФормаДокумента"
			Тогда
			ДЗК_ДобавлениеРеквизитов.ДобавлениеРеквизитаНаФорму(Форма);   
		КонецЕсли;
	КонецЕсли;	
	//-- Жоголев Д Задача: Добаление реквизитов на форму 27.02.25	
